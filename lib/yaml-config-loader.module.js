...
let YamlConfigLoaderModule = YamlConfigLoaderModule_1 = class YamlConfigLoaderModule {
    ...
    static createConfigLoaders(env, transformers) {
        ...
        return keys.map((key) => {
            return (0, config_1.registerAs)(key, async () => {
                const filePath = (0, path_1.join)(process.cwd(), 'dist', 'config', `${key}.config`);
                try {
                    const importedModule = await Promise.resolve(`${filePath}`).then(s => require(s));
